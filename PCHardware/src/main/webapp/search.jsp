...
<div class="sideBar">
    <form action="/Controller" method="post">
        ...
        <fieldset>
            <legend>${price}</legend>
            ${max}: <input type="number" name="maxPrice" min="0" max="400000" step="10000" value="${param.maxPrice}"/><br>
            ${min}: <input type="number" name="minPrice" min="0" max="400000" step="10000" value="${param.minPrice}"/><br>
        </fieldset>
        <fieldset>
            <legend>${vendorMessage}</legend>
            <c:forEach var="vendor" items="${setOfVendors}">
                ${vendor} <input type="checkbox" name="${vendor}" ${param[vendor] == 'on' ? 'checked' : '' }/><br>
            </c:forEach>
        </fieldset>
        <input type="submit"/>
    </form>
